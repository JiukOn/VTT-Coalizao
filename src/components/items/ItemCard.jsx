...
export default function ItemCard({ item, onSelect, onContextMenu }) {
  const rarity = (item.rarity || 'comum').toLowerCase()
    ...
    >
      <div className="entity-card-header">
        ...
        <div style={{ flex: 1, overflow: 'hidden' }}>
          <div className="entity-card-name" style={{ fontSize: '0.9rem' }}>{item.name}</div>
      ...
      {item.bonuses && Object.keys(item.bonuses).length > 0 && (
        <div style={{ marginTop: 6, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
          {Object.entries(item.bonuses).filter(([, v]) => v).map(([attr, val]) => (
