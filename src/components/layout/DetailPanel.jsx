...
import { X, Heart, Shield, Zap, Swords, User, Eye, Activity, ScrollText } from 'lucide-react'
...
export default function DetailPanel({ isOpen, onToggle, entity, onAddToTable, tableEntities = [], onUpdateTableEntity }) {
  ...
  useEffect(() => {
    if (entity) {
      const defaultTab = (entity.entityType === 'npc' || entity.entityType === 'creature') ? 'ficha' : 'info'
      setActiveTab(defaultTab)
    }
  }, [entity?.id, entity?.tableId, entity?.entityType])
  ...
  const hasSheet = entity?.entityType === 'npc' || entity?.entityType === 'creature'
  const tabs = hasSheet ? ['ficha', 'efeitos'] : ['info', 'atributos', 'efeitos']
  const tabLabels = {
    ficha: 'Ficha',
    info: 'Info',
    atributos: 'Atributos',
    efeitos: `Efeitos${activeEffects.length > 0 ? ` (${activeEffects.length})` : ''}`,
  }
          ...
          {entity ? (
            <>
              ...
              <div className="detail-panel-content">
                {/* FICHA TAB — full NPCSheet or CreatureSheet */}
                {activeTab === 'ficha' && (
                  entity?.entityType === 'npc'
                    ? <NPCSheet npc={entity} />
                    : <CreatureSheet creature={entity} />
                )}
