...
import { validateMasterMessage, validatePlayerMessage } from '../../shared/schemas/wsMessages.js'
...
function cleanupRoom(code) {
  ...
  if (!room.host && room.players.size === 0) {
    if (room.orphanTimer) clearTimeout(room.orphanTimer)
    rooms.delete(code)
...
function onClientDisconnect(clientId) {
  ...
  if (role === 'host') {
    ...
    // Set a 5-minute timeout to clean up the room if host does not reconnect
    if (room.orphanTimer) clearTimeout(room.orphanTimer)
    room.orphanTimer = setTimeout(() => {
      if (!room.host) {
        for (const player of room.players.values()) {
          safeSend(player.ws, { type: 'error', message: 'Sessão encerrada por inatividade do Mestre.' })
          player.ws.close()
        }
        room.players.clear()
...
function handleMessage(clientId, ws, msg) {
  ...
  if (!msg || typeof msg !== 'object' || !msg.type) {
    console.warn(`[Relay] Invalid payload from client ${clientId}:`, msg)
    safeSend(ws, { type: 'error', message: 'Payload inválido.' })
    return
  }

  let isValid = false
  if (meta.role === 'host') {
    if (msg.type === 'ping') isValid = true
    else isValid = validateMasterMessage(msg).success
  } else if (meta.role === 'player') {
    isValid = validatePlayerMessage(msg).success
  } else {
    if (msg.type === 'host_hello') isValid = validateMasterMessage(msg).success
    else if (['join', 'get_characters', 'ping'].includes(msg.type)) isValid = validatePlayerMessage(msg).success
  }
  ...
  switch (msg.type) {
    ...
    case 'chat_message': {
      ...
      if (meta.role === 'player') {
        const player = room.players.get(clientId)
        if (player) msg.sender = player.playerName
      ...
      if (msg.isWhisper && msg.target) {
        let targetWs = null
        if (msg.target === 'host' || msg.target === 'Mestre') {
          targetWs = room.host?.ws
        } else {
          for (const p of room.players.values()) {
            if (p.playerName === msg.target) {
              targetWs = p.ws
              break
            }
          }
        ...
        if (room.host?.ws.readyState === 1) room.host.ws.send(out)
        for (const p of room.players.values()) {
