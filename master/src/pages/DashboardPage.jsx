...
export default function DashboardPage({ onSelectEntity, onEntityContextMenu, tableEntities = [], setTableEntities, onUpdateTableEntity }) {
  ...
  const { undo, redo, push, canUndo, canRedo } = useUndoStack({
    onUndo: (state) => {
      if (state.type === 'table_update') setTableEntities?.(state.inverse)
    },
    onRedo: (state) => {
      if (state.type === 'table_update') setTableEntities?.(state.data)
    }
  })
  ...
  const handleAddEffect = (tableId) => {
    ...
    setTableEntities?.(prev => {
      ...
      push({ type: 'table_update', description: 'Add effect', inverse: prev, data: next })
      return next
    })
  ...
  const updateEffectDuration = (tableId, effectIdx, delta) => {
    setTableEntities?.(prev => {
      ...
      push({ type: 'table_update', description: 'Update effect duration', inverse: prev, data: next })
      return next
    })
  }
  ...
  const removeEffect = (tableId, effectIdx) => {
    setTableEntities?.(prev => {
      ...
      push({ type: 'table_update', description: 'Remove effect', inverse: prev, data: next })
      return next
    })
  }
    ...
    <div className="dashboard-page">
      ...
      <div className="dashboard-section">
        ...
        <div className="dashboard-actions-grid">
          ...
          <button className="dashboard-action-btn" onClick={() => setGeneratorModalOpen(true)} style={{ borderColor: 'var(--accent-primary)' }}>
        ...
        <div className="dashboard-section" style={{ border: '1px solid var(--border-color)', padding: 'var(--space-md)', borderRadius: '8px', background: 'var(--bg-secondary)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button className="btn btn-icon btn-sm" onClick={undo} disabled={!canUndo} title="Undo (Ctrl+Z)"><Undo2 size={16} /></button>
              <button className="btn btn-icon btn-sm" onClick={redo} disabled={!canRedo} title="Redo (Ctrl+Y)"><Redo2 size={16} /></button>
              <button 
                className="btn btn-sm btn-primary" 
                onClick={() => {
                  setTableEntities?.(prev => {
                    const next = [...prev].sort((a, b) => (Number(b.initiative) || 0) - (Number(a.initiative) || 0))
                    push({ type: 'table_update', description: 'Sort initiative', inverse: prev, data: next })
                    return next
                  })
                }}
              >
              ...
              <button className="btn btn-sm btn-secondary" onClick={() => setTableEntities?.(prev => { push({ type: 'table_update', description: 'Clear table', inverse: prev, data: [] }); return []; })}>Clear Table</button>
            </div>
          </div>
                  ...
                  <label style={{ fontSize: '0.7rem', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        ...
                        setTableEntities?.(prev => {
                          const next = prev.map(ent => ent.tableId === entity.tableId ? { ...ent, initiative: val } : ent)
                          push({ type: 'table_update', description: 'Update initiative', inverse: prev, data: next })
                          return next
                        })
                      }} 
                    />
                  ...
                  <label style={{ fontSize: '0.7rem', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        const val = e.target.value
                        setTableEntities?.(prev => {
                          const next = prev.map(ent => ent.tableId === entity.tableId ? { ...ent, hp: val } : ent)
                          push({ type: 'table_update', description: 'Update HP', inverse: prev, data: next })
                          return next
                        })
                      }} 
                    />
                  ...
                  <label style={{ fontSize: '0.7rem', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        const val = e.target.value
                        setTableEntities?.(prev => {
                          const next = prev.map(ent => ent.tableId === entity.tableId ? { ...ent, dp: val } : ent)
                          push({ type: 'table_update', description: 'Update DP', inverse: prev, data: next })
                          return next
                        })
                      }} 
                    />
                  ...
                  <label style={{ fontSize: '0.7rem', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        const val = e.target.value
                        setTableEntities?.(prev => {
                          const next = prev.map(ent => ent.tableId === entity.tableId ? { ...ent, aura: val } : ent)
                          push({ type: 'table_update', description: 'Update aura', inverse: prev, data: next })
                          return next
                        })
                      }} 
                    />
