...
import { useWebSocket, WS_STATUS } from '@shared/hooks/useWebSocket.js'
import { useLocalStorage } from '@shared/hooks/index.js'
...
export function ServerProvider({ children }) {
  ...
  const [customServerUrl, setCustomServerUrl] = useLocalStorage('vtt_custom_server_url', '')

  const [wsUrl, setWsUrl]               = useState(() => {
    if (!relayMode) {
      return customServerUrl ? customServerUrl : `ws://localhost:${serverPort || 3001}`
    }
    return null
  })
  ...
  const handleMessage = useCallback((msg) => {
    switch (msg.type) {
      case 'host_welcome':
        ...
        if (Array.isArray(msg.ips)) setServerIps(msg.ips)
      ...
      case 'player_left':
        ...
        break
      case 'player_dice':
        // Dispatched so components can listen via window.addEventListener('vtp:player_dice', ...)
      ...
      case 'token_move':
        ...
        break
      case 'map_ping':
        window.dispatchEvent(new CustomEvent('vtp:map_ping', { detail: msg }))
        break
      case 'player_notes':
  ...
  const connect = useCallback((port = serverPort) => {
    ...
    const url = customServerUrl ? customServerUrl : `ws://localhost:${port}`
    setWsUrl(url)
    setPlayers([])
    setSessionCode('')
  }, [serverPort, customServerUrl])
   ...
   * type: 'map_update' | 'entity_update' | 'turn_change' | 'combat_event' | 'game_state_update' | 'roster_update'
   */
  const broadcast = useCallback((type, data) => {
    if (data && typeof data === 'object' && !Array.isArray(data) && ('characters' in data || 'data' in data)) {
