...
function StepIdentity({ character, updateField, toggleTendency, onPrimarySpeciesChange, newAlignment, setNewAlignment, addAlignment, removeAlignment }) {
  ...
  return (
    <div className="form-grid">
      ...
      {/* Token Image */}
      <div className="form-group">
        <label className="input-label">Imagem do Token</label>
        <input
          className="input"
          type="file"
          accept="image/*"
          onChange={async (e) => {
            ...
            reader.onload = async () => {
              const compressed = await compressToWebP(reader.result, { maxWidth: 512, maxHeight: 512, quality: 0.85 })
              updateField('tokenImage', compressed)
            }
            reader.readAsDataURL(file)
          }}
        />
        {character.tokenImage && (
          <img src={character.tokenImage} alt="Token" style={{ width: 40, height: 40, marginTop: 8, borderRadius: 4, objectFit: 'cover' }} />
        )}
      </div>
      ...
      <div className="form-group">
        <label className="input-label" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span>🔑 Senha de Acesso do Jogador</span>
          <button
            type="button"
        ...
        <input
          ...
          placeholder="Opcional — Protege o herói com senha"
        />
      </div>
    ...
    <div className="step-review">
      ...
      <div className="review-section">
        ...
        <div className="review-grid">
          ...
          <ReviewField label="Senha de Acesso" value={character.password ? '🔒 ' + character.password : '🔓 Livre (sem senha)'} />
        </div>
