        ...
        <input
          ...
          onChange={async (e) => {
            ...
            reader.onload = async () => {
              ...
              set('tokenImage', compressed)
            }
            reader.readAsDataURL(file)
          }}
        />
        {form.tokenImage && (
          <img src={form.tokenImage} alt="Token" style={{ width: 40, height: 40, marginTop: 8, borderRadius: 4, objectFit: 'cover' }} />
        )}
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div className="form-group">
              ...
              <select className="input select" value={form.type || ''} onChange={e => set('type', e.target.value)}>
                ...
                {NPC_TYPES.map(nt => <option key={nt} value={nt}>{nt}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label className="input-label">Localização</label>
              <input className="input" value={form.location || ''} onChange={e => set('location', e.target.value)} placeholder="Capital dos Elfos..." />
            </div>
          </div>
          <div className="form-group">
