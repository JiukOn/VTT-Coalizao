...
export default function CombatResolver({ entities = [], onLogEntry, onApplyDamage }) {
  ...
  const handleResolve = () => {
    ...
    setTimeout(() => {
      ...
      // Telemetria assíncrona não-bloqueante para o SQLite WAL
      try {
        fetch('/api/engine/db/log-combat', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            session_code: 'LOCAL_SESSION',
            round_number: 1,
            attacker: an,
            target: dn,
            action_type: attackType,
            dice_result: res.attackRoll || 10,
            damage_dealt: res.hit ? (res.damageDealt || 0) : 0,
            is_crit: res.attackRoll === 20,
            details: `${res.typeLabel}: ${res.attackTotal} vs ${res.defendTotal}`,
          }),
        }).catch(() => {})
      } catch {
        // Ignorar se motor offline
      }
