...
export default function Sidebar({ isOpen, onToggle, activeTab, onSelectEntity }) {
  ...
  useEffect(() => {
    ...
    load()
  }, [activeTab, isOpen, activeCampaign, config])

  const filtered = entities.filter(e => {
    const name = t(e[config?.nameField]) || ''
        ...
        <>
          ...
          <div className="sidebar-content">
            {!config ? (
              ...
              <p className="text-muted" style={{ padding: '12px', fontSize: '0.8rem', textAlign: 'center' }}>{ui('carregando')}</p>
            ) : filtered.length === 0 ? (
              <p className="text-muted" style={{ padding: '12px', fontSize: '0.8rem', textAlign: 'center' }}>{ui('nenhum_encontrado')}</p>
            ) : (
              <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
                {filtered.map((entity, idx) => {
                  const Icon = config.icon
                  const name = t(entity[config.nameField]) || t(entity.name) || 'Sem nome'
                  const sub = t(entity[config.subField]) || ''
                  return (
                    <li
                      key={entity.id || idx}
