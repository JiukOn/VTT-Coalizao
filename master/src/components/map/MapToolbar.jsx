...
export default function MapToolbar({
  ...
  const tools = [
    ...
    { id: 'wall',    icon: Minus,         label: 'Parede sólida (bloqueia visão e movimento)' },
    { id: 'door',    icon: DoorOpen,      label: 'Porta interativa (clique para alterar estado)' },
    { id: 'window',  icon: Square,        label: 'Janela / Vidro (visão passa, movimento bloqueia)' },
    { id: 'cliff',   icon: Layers,        label: 'Penhasco / Sacada (visão unidirecional)' },
    { id: 'aoe',     icon: Sparkles,      label: 'Área de Efeito / Magia (AoE)' },
    { id: 'marker',  icon: MapPin,        label: 'Marcador / Nota Secreta do Mestre' },
      ...
      {activeTool === 'aoe' && (
        <>
          <select
            ...
            className="input"
            style={{ padding: '2px 6px', fontSize: '0.75rem', height: 26, background: 'var(--bg-tertiary)' }}
          >
          ...
          <select
            ...
            style={{ padding: '2px 6px', fontSize: '0.75rem', height: 26, background: 'var(--bg-tertiary)' }}
          >
            <option value="3">3 metros (2 quad.)</option>
            <option value="4.5">4.5 metros (3 quad.)</option>
            <option value="6">6 metros (4 quad.)</option>
            <option value="9">9 metros (6 quad.)</option>
            <option value="12">12 metros (8 quad.)</option>
            <option value="15">15 metros (10 quad.)</option>
          </select>
          ...
          {onClearAoe && (
            <button
              className="btn btn-ghost btn-sm"
              onClick={onClearAoe}
              title="Remover template de magia do mapa"
              style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: '0.72rem' }}
            >
              <Trash2 size={11} /> Limpar Magia
            </button>
          )}
        </>
      )}
      ...
      {fogEnabled && (
        <>
          ...
          {(activeTool === 'fog_reveal' || activeTool === 'fog_hide') && onFogBrushSizeChange && (
            ...
            >
              <option value="1">Pincel 1q (1.5m)</option>
              <option value="3">Pincel 3q (4.5m)</option>
              <option value="5">Pincel 5q (7.5m)</option>
            </select>
          )}
