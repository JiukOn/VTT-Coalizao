...
const CURRENT_SEED_VERSION = '12.0.0' // Bump: boss creature stats (15) + lancax creature + lancax species
  ...
  try {
    await db.transaction('rw',
      ...
      async () => {
        ...
        await db.creatures.bulkPut(BASE_CREATURES.map(mapCreature))
        ...
        await db.abilities.bulkPut(abilitiesWithFlag)
        ...
        await db.ambients.bulkPut(BASE_DOMAINS.map(d => ({ ...d, ambientId: d.id, isCustom: 0 })))
        console.log(`[Seeder] ✓ ${BASE_DOMAINS.length} ambientes`)

        // Classes
        await db.classes.clear()
        await db.classes.bulkPut(CLASS_DATA.map(c => ({ ...c, classId: c.id, isCustom: c.isCustom ? 1 : 0 })))
        console.log(`[Seeder] ✓ ${CLASS_DATA.length} classes`)

        // Species
        await db.species.clear()
        await db.species.bulkPut(BASE_SPECIES.map(s => ({ ...s, speciesId: s.id, isCustom: s.isCustom ? 1 : 0 })))
        console.log(`[Seeder] ✓ ${BASE_SPECIES.length} espécies`)

        // Auras
        await db.auras.clear()
        await db.auras.bulkPut(BASE_AURAS.map(a => ({ ...a, auraId: a.id, isCustom: a.isCustom ? 1 : 0 })))
        ...
        await db.characters.bulkPut(heroesForDB)
        ...
        if (existingSessions === 0) {
          await db.sessionNotes.bulkPut(BASE_SESSIONS)
