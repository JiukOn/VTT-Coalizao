/* visionUtils.js — Canonical shared ray-casting + vision utilities for map rendering */
...
export function isBlockedByWall(ox, oy, tx, ty, walls = []) {
  for (const w of walls) {
    // 1. Open doors do not block vision
    if (w.isDoor && w.doorState === 'open') continue

    // 2. Windows / Glass walls allow line of sight through them
    if (w.isWindow || w.wallType === 'window') continue

    // Check intersection with ray
    if (segmentsIntersect(ox, oy, tx, ty, w.x1, w.y1, w.x2, w.y2)) {
      // 3. One-Way Cliffs / Ledges: allow vision when looking down from high side
      if (w.isOneWay || w.wallType === 'cliff') {
        const dx = w.x2 - w.x1
        const dy = w.y2 - w.y1
        // Cross product determines if origin (ox, oy) is on the high side (left side of stroke)
        const sideOrigin = dx * (oy - w.y1) - dy * (ox - w.x1)
        if (sideOrigin > 0) {
          continue // Observer is on high ground: vision is unobstructed
        }
      }
      return true
    }
...
export function hasLineOfSight(ox, oy, tx, ty, walls = []) {
  return !isBlockedByWall(ox, oy, tx, ty, walls)
}
