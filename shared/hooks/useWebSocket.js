 ...
 */
export function useWebSocket(url, onMessage, { autoReconnect = true, onReconnect } = {}) {
  ...
  const doConnect = useCallback((wsUrl) => {
    ...
    // Defer status transition to avoid synchronous setState inside useEffect
    queueMicrotask(() => setStatus(WS_STATUS.CONNECTING))
